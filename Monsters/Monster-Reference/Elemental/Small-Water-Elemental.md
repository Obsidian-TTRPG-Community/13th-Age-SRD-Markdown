---
aliases: [Small Water Elemental]
created: 2023-05-23
description: 
image: 
level: 3
publish: 
role: blocker
strength: normal
tags: ["13A/Bestiary/elemental", "13A/Monsters/Type/blocker"]
type: elemental
updated: 2023-05-24
---
```statblock
layout: Basic 13th Age Monster Layout
columns: 1
name: "Small Water Elemental"
size: "normal"
level: "3"
levelOrdinal: "3rd"
role: "blocker"
type: "elemental"
initiative: "7"
actions:
    - name: "Surge +7 vs. AC (up to 2 enemies)"
      desc: "8 damage"
      traits:
          - name: "Miss"
            desc: "The elemental heals 3 hp."
traits:
    - name: "Liquid empowerment"
      desc: "The water elemental gains a +2 bonus to attacks and all defenses while it’s in contact with a body of water, or while it’s nearby a sizeable body of water. A bucket or a bathtub of liquid doesn’t count; it must be at least a pond, creek, or maybe a large fountain. If the water elemental moves far away from the body of water the empowerment ends."
    - name: "Resist weapon damage 16+"
      desc: "When a weapon attack targets this creature, the attacker must roll a natural 16+ on the attack roll or it only deals half damage."
ac: "18"
pd: "16"
md: "12"
hp: "40"
```

